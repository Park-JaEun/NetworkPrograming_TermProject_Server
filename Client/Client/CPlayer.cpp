...
<<<<<<< HEAD
	...
	if (m_EffectAnimator != nullptr)
		m_EffectAnimator->update();	// ÀÌÆåÆ® ¾Ö´Ï¸ÞÀÌÅÍ ¾÷µ¥ÀÌÆ®

	GetAnimator()->update();		// ¾Ö´Ï¸ÞÀÌÅÍ ¾÷µ¥ÀÌÆ®
...
=======
	if (m_EffectAnimator != nullptr)
		m_EffectAnimator->update();	// ÀÌÆåÆ® ¾Ö´Ï¸ÞÀÌÅÍ ¾÷µ¥ÀÌÆ®
	GetAnimator()->update();		// ¾Ö´Ï¸ÞÀÌÅÍ ¾÷µ¥ÀÌÆ®
>>>>>>> parent of 68322ae ([ê³µí†µ] í”Œë ˆì´ì–´ ìœ„ì¹˜ ì˜ˆì¸¡ê³¼ ë³´ê°„ ì¶”ê°€, ëª¨ë“  í´ë¼ì´ì–¸íŠ¸ í‚¤ìž…ë ¥ ë™ê¸°í™” êµ¬í˜„)
}
...
void CPlayer::render(HDC _dc)
{
	...
	if (GetName() == L"Player" + std::to_wstring(id)) {
		...
		if (AddFontResourceEx(fontPath, FR_PRIVATE, 0) > 0) {
			...
			lf.lfHeight = 30;  // ÆùÆ® Å©±â ¼³Á¤
			lf.lfWeight = 50;  // ÆùÆ® µÎ²² ¼³Á¤
...
void CPlayer::CreateMinji()
{
	...
	pAnimator->Play(L"Minji_Idle_Right", true);	// ÇöÀç ¾Ö´Ï¸ÞÀÌ¼Ç ÁöÁ¤
...
void CPlayer::CreateHannie()
{
	...
	pAnimator->Play(L"Hannie_Idle_Right", true);	// ÇöÀç ¾Ö´Ï¸ÞÀÌ¼Ç ÁöÁ¤
...
void CPlayer::CreateDanielle()
{
	...
	pAnimator->Play(L"Danielle_Idle_Right", true);	// ÇöÀç ¾Ö´Ï¸ÞÀÌ¼Ç ÁöÁ¤
...
void CPlayer::CreateHyein()
{
	...
	pAnimator->Play(L"Hyein_Idle_Right", true);	// ÇöÀç ¾Ö´Ï¸ÞÀÌ¼Ç ÁöÁ¤
...
void CPlayer::CreateHaerin()
{
	...
	pAnimator->Play(L"Haerin_Idle_Right", true);	// ÇöÀç ¾Ö´Ï¸ÞÀÌ¼Ç ÁöÁ¤
...
{
	// º¸°£ ·ÎÁ÷
	// ÀÌÀü Æ÷Áö¼Ç°ú ÇöÀç Æ÷Áö¼Ç »óÅÂ »çÀÌ¸¦ º¸°£
	Vec2 vPos = GetPos();
